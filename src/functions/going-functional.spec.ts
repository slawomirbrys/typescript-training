...
describe('Functional Programming', function(){
	...
	describe('unary function', function(){
		it('can override arguments passed down to a function', function(){
			...
			expect(unaryParseInt('10')).toEqual(10);
	...
	describe('compose function', function(){
		...
		it('can execute sequence of N operations as a new reusable function', function(){
			...
			function compose(...fns: Function[]){
				return function(value: any){
					return fns.slice().reverse().reduce(function(res, fn){
						return fn(res);
					}, value);
				}
			}
		...
		it('can be used for complex data processing', function(){
			...
			// write 'getUsersByNationality' closure function
			// it accepts the nationality to filter by
			// and returns a function that, given users list, returns users fltered by nationality

			function getUsersByNationality(nationality){
				return function(users){
					return users.filter(function(user){
						return user.nationality === nationality;
					});
				};
			}

			expect(typeof getUsersByNationality).toEqual("function");
			expect(getUsersByNationality.length).toEqual(1);

			var getDEUsers = getUsersByNationality('DE');
			expect(typeof getDEUsers).toEqual("function");
			expect(getDEUsers.length).toEqual(1);
			expect(getDEUsers(users).length).toEqual(39);

			var getUKUsers = getUsersByNationality('UK');
			expect(typeof getUKUsers).toEqual("function");
			expect(getUKUsers.length).toEqual(1);
			expect(getUKUsers(users).length).toEqual(30);
			...
			// write 'getSalaries' function that, given users list,
			// will return a list of their salary attribute values list

			var getSalaries = function(users){
			...
			// use function composition to: get (1) half of (2) the sum of (3) all salaries of (4) only DE users
			var halfDESalary = compose(half, sum, getSalaries, getDEUsers);
			expect(halfDESalary(users)).toEqual(114576);

			// use function composition to: get (1) the sum of (2) all salaries of (3) only UK users
			var fullUKSalary = compose(sum, getSalaries, getUKUsers);
			expect(fullUKSalary(users)).toEqual(163734);
